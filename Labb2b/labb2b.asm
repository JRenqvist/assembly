...
; Definitions for ping-pong variables
DIODES		.equ	0x20001000
PLAYER_RIGHT	.equ	0x20001010
PLAYER_LEFT 	.equ	0x20001020
DIRECTION	.equ	0x20001030
SERVE_STATUS	.equ	0x20001040
...
main:
	...
	; Reset score
	mov r0, #(PLAYER_RIGHT & 0xffff)
	movt r0, #(PLAYER_RIGHT >> 16)
	mov r1, #0
	str r1, [r0]

	mov r0, #(PLAYER_LEFT & 0xffff)
	movt r0, #(PLAYER_LEFT >> 16)
	mov r1, #0
	str r1, [r0]

	; Set serve status to true
	...
	mov r0, #(DIRECTION & 0xffff)
	movt r0, #(DIRECTION >> 16)
	mov r1, #0x00
	str r1, [r0]

	; Set diode at bit 0 to 1 to indicate right serve
	mov r0, #(DIODES & 0xffff)
	movt r0, #(DIODES >> 16)
	mov r1, #1
	str r1, [r0]
...
mainmove:
	bl moveball		; Moves the ball in the correct direction
	mov r1, #750
	bl DELAY

	; Make sure serve is still false, incase we clicked a button to make serve true
	mov r1, #(SERVE_STATUS & 0xffff)
	movt r1, #(SERVE_STATUS >> 16)
	ldr r2, [r1]
	cmp r2, #0xFF
	...
	; If ball goes out of bounds, set serve to true
	mov r1, #(DIODES & 0xffff)
	movt r1, #(DIODES >> 16)
	ldr r2, [r1]
...
moveball: ; Calls appropriate function if we want to move the ball to the left of right
...
moveballright:
	...
	; Write updated diode config to DIODES
	str r1, [r0]

	; Write diode config to port B
...
setserveleft:	; Gets called when right misses ball
	...
	; Set diode at bit 7 to 1 to indicate left serve
	mov r0, #(DIODES & 0xffff)
	movt r0, #(DIODES >> 16)
	mov r1, #0x80
	str r1, [r0]
...
setserveright:	; Gets called when left misses ball
	...
	; Set diode at bit 0 to 1 to indicate left serve
	mov r0, #(DIODES & 0xffff)
	movt r0, #(DIODES >> 16)
	mov r1, #1
	str r1, [r0]
...
; Input: Player to add 1 point to stored in r0 (0x00 left, 0xFF right)
addpoints:
	...
	mov r1, #(PLAYER_LEFT & 0xffff)
	movt r1, #(PLAYER_LEFT >> 16)
	ldrb r2, [r1]
	add r2, r2, #1
	str r2, [r1]
...
addpointsright:
	...
	mov r1, #(PLAYER_RIGHT & 0xffff)
	movt r1, #(PLAYER_RIGHT >> 16)
	ldrb r2, [r1]
	add r2, r2, #1
	str r2, [r1]
...
;; Helper functions for intgpiod/e
notserveright:
	...
	ldr r2, [r1]
	cmp r2, #1
	bne intsetserveleft	; Branches if bit 0 is not 1, which means we did not hit the ball

	; If we get here, it means we hit the ball correctly, change the direction
	mov r1, #(DIRECTION & 0xffff)
	movt r1, #(DIRECTION >> 16)
	mov r2, #0x00
	str r2, [r1]
...
notserveleft:
	...
	ldr r2, [r1]
	cmp r2, #0x80
	bne intsetserveright	; Branches if bit 7 is not 1, which means we did not hit the ball
...
intsetserveright:	; Gets called when left misses ball
...
intsetserveleft:	; Gets called when right misses ball
...
intgpiod:	; RIGHT PLAYER
	...
	mov r2, #0xFF
	cmp r2, r1
	bne notserveright	; Branches if serve is false

	; Check the DIODE configuration to see if correct player has serve
	mov r1, #(DIODES & 0xffff)
	movt r1, #(DIODES >> 16)
	ldr r2, [r1]
	mov r3, #1
	cmp r3, r2
	mov r0, #0x00	; Input for addpoints function
	bne addpoints	; Branch and add score to left player

	; Set SERVE_STATUS to false
	mov r1, #(SERVE_STATUS & 0xffff)
	movt r1, #(SERVE_STATUS >> 16)
	mov r2, #0x00
	str r2, [r1]
...
intgpioe:	; LEFT PLAYER
	...
	cmp r3, r2
	mov r0, #0xFF
	bne addpoints	; Branch and add score to right player
